  ...
  template: `
    <article class="legal-page pt-28 sm:pt-32 pb-16 px-5 sm:px-8 lg:px-12">
      <div class="max-w-[800px] mx-auto">
        ...
        <h1 class="text-[clamp(1.8rem,4vw,2.8rem)] font-black leading-tight mb-2">{{ i18n.t().terms.title }}</h1>
        <p class="text-text-muted text-[0.85rem] mb-10">{{ i18n.t().legal.lastUpdated }}</p>

        <div class="legal-content">
          @for (section of i18n.t().terms.sections; track section.title) {
            <section>
              <h2>{{ section.title }}</h2>
        ...
        <div class="mt-12 pt-8 border-t border-border-subtle">
          <p class="text-text-dim text-[0.8rem]">{{ i18n.t().legal.seeAlsoPrivacy }} <a routerLink="/politica-de-privacidade" class="text-verde-bio no-underline hover:underline">{{ i18n.t().legal.privacyLink }}</a>.</p>
        </div>
      </div>
    </article>
    <app-footer />
